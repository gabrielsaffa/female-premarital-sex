...
### this script contains the code for the data prep (coding & imputation) and modeling procedure ###
...
norms <- read.csv ("sex_norms_data.csv", header=TRUE)
str (norms)

pop <- norms$Population # store population column into an object

norms2 <- norms[,-c(1:3)] # remove population column and GPS coordinates

norms2[c(26,36,40,41,43,44,46,47,55,56,57,58,61,65,66,67,68,153,184),12] <- 7 # change to indirect dowry; see Schlegel (1991)

str (norms2) # integers, convert to factors, fem_cont to numeric

norms2$sex_norms <- as.factor (norms2$sex_norms)
norms2$ext_aff <- as.factor (norms2$ext_aff)
norms2$inher_land <- as.factor (norms2$inher_land)
norms2$inher_mov <- as.factor (norms2$inher_mov)
norms2$dir_care_inf <- as.factor (norms2$dir_care_inf)
norms2$dir_care_child <- as.factor (norms2$dir_care_child)
norms2$fem_cont <- as.numeric (norms2$fem_cont)
norms2$sex_rat <- as.factor (norms2$sex_rat)
norms2$marr_arr <- as.factor (norms2$marr_arr)
norms2$dec_mak <- as.factor (norms2$dec_mak)
norms2$trans_resid <- as.factor (norms2$trans_resid)
norms2$marr_tr <- as.factor (norms2$marr_tr)
...
write.csv (norms_imputed_pop, file="norms_imputed.csv") # save the imputed data


######################
### data re-coding ###

norms <- read.csv ("norms_imputed.csv", header=TRUE) # load the imputed data
...
write.csv (norms_data, "norms_final.csv")
...
N_pop <- nrow (norms) # index of societies


# standardize variables by centering and dividing by 2 SD; see Gelman (2008)
...
pop_obs <- norms$pop
tree_trimmed <- keep.tip (tree, pop_obs) # combine tips of the phylogeny with societies in the sample
...
d1_norm$Dmat <- Dmat[pop_obs,pop_obs]/max(Dmat)
...
d3_norm$Dmat <- Dmat[pop_obs,pop_obs]/max (Dmat) # include distance matrix
