...
### this script contains the code to reproduce all the figures included in the study ###
...
norms <- read.csv ("sex_norms_data.csv", header=TRUE) # load the raw data

norms$sex_norms[is.na(norms$sex_norms)] <- 9 # replace NAs with a number
norms$sex_norms <- as.factor (norms$sex_norms)


world <- map_data ("world")

labs_norm <- c ("Early marriage","Strongly sanctioned","Weakly sanctioned","Sanctions if pregnancy","Trial marriage","No sanctions","NA")
cols_norm <- c ("darkorange4","goldenrod4","goldenrod3","goldenrod2","goldenrod1","gold","black")

map_norm <- ggplot() + geom_polygon(data=world, aes(x=long, y=lat, group=group), fill="grey85", color="grey85") + theme_classic() + labs (x="", y="") + coord_fixed(1.3) + geom_point(data=norms, aes(x=long, y=lat, fill=as.factor(norms[,4]), shape=as.factor(norms[,4])), size=3, colour="black") + theme (legend.position="bottom", legend.title=element_blank(), legend.text=element_text(size=16), axis.text.x=element_text(size=16), axis.text.y=element_text(size=16), text=element_text(size=16), plot.margin=unit(c(0,1,0,0),"cm")) + scale_fill_manual (labels=labs_norm, values=cols_norm) + scale_colour_manual (labels=labs_norm, values=cols_norm) + scale_shape_manual (values=c(21,21,21,21,21,21,1), guide=FALSE)
map_norm
...
plot (NULL, xlim=c(0,1), ylim=c(0,5), xlab="", ylab="", main="", cex.axis=1.35)
mtext("a", side = 3, line=0.25, cex = 1.5, adj=0)
mtext ("covariance", 2, line=2.75, cex=1.35)
...
### density plot (Figure 2b)

norm_post_g <- read.csv ("norm_post_g.csv", header=TRUE)

norm_post_g <- norm_post_g [,7:17]
...
norm <- ggplot(norm_post_g, aes(x=est, y=var)) + labs (y="", x="effect size") + geom_vline(xintercept=0, alpha=1, linetype="longdash") + geom_hline(yintercept=1:11, alpha=1, colour="gray85") + geom_density_ridges2(aes(fill=var, color=var), lwd=0.8, scale=2.5, rel_min_height=0.01, alpha=0.6) + scale_fill_manual(values=cols, labels=names) + scale_color_manual(values=cols, labels=names) + theme_classic(base_size=12) + theme(legend.position="none",axis.text.x=element_text(size=15), axis.text.y=element_text(size=15), text = element_text(size=15),  plot.margin=unit(c(1.5,1,1.5,0.5),"cm")) + ggtitle ("b") + scale_y_discrete (limits=names, labels=names, expand=c(0.05,0.0)) + coord_cartesian (clip="off") + annotate("text", x=2.6, y=1.3:11.3, colour=c("turquoise2","turquoise3","turquoise4","#663300","orange4","#993300","gold","yellow2","magenta2","magenta4","deeppink4"), size=6, fontface="bold", label=c("29.42","53.60","48.30","32.73",">99",">99",">99","98.85","89.75","33.70",">99"))
...
### build posterior predictive plot
...

{
  ...
  plot (NULL, type="n", xlab="effect size", ylab="", xlim=c(-2,2), ylim=c(0,1), yaxt="n", xaxt="n", cex.lab=1.5)
  axis(1, at=seq(-2,2,length.out = 5), cex.axis=1.5)
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("extramarital sex", cex=1.25, line=0.5)
  ...
  ext_aff <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    ext_aff[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  ext_aff <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    ext_aff[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, at=seq(-2,2,length.out = 5), cex.axis=1.5)
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("direct care", cex=1.25, line=0.5)
  ...
  dir_care <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    dir_care[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  dir_care <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    dir_care[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  plot (NULL, type="n", xlab="", ylab="", xlim=c(0,1), ylim=c(0,1), yaxt="n", xaxt="n", cex.lab=1.5)
  axis(1, cex.axis=1.5, at=c(0.1,0.9), labels=c("Absent", "Present"))
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("inheritance", cex=1.25, line=0.5)
  ...
  inher <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    inher[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  inher <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    inher[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, at=seq(-2,2,length.out = 5), cex.axis=1.5)
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("female contribution", cex=1.25, line=0.5)
  ...
  fem_cont <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    fem_cont[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  fem_cont <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    fem_cont[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, at=seq(-2,2,length.out = 5), cex.axis=1.5)
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("sex ratio", cex=1.25, line=0.5)
  mtext ("probability", 2, cex=1.25, line=2.75)
  ...
  sex_rat <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    sex_rat[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  sex_rat <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    sex_rat[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, at=seq(-2,2,length.out = 5), cex.axis=1.5)
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("parental control", cex=1.25, line=0.5)
  ...
  marr_arr <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    marr_arr[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  marr_arr <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    marr_arr[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, at=seq(-2,2,length.out = 5), cex.axis=1.5)
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("decision makers", cex=1.25, line=0.5)
  ...
  dec_mak <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    dec_mak[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  dec_mak <- matrix(0, nrow=100, ncol=length(seq_x))
  for (i in 1:100) {
    dec_mak[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, cex.axis=1.5, at=c(0.1,0.9), labels=c("Absent", "Present"))
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("patrilocality", cex=1.25, line=0.5)
  ...
  p_loc <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    p_loc[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  p_loc <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    p_loc[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, cex.axis=1.5, at=c(0.1,0.9), labels=c("Absent", "Present"))
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("bride-price", cex=1.25, line=0.5)
  mtext ("probability", 2, cex=1.25, line=2.75)
  ...
  bride_pr <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    bride_pr[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  bride_pr <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    bride_pr[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  axis(1, cex.axis=1.5, at=c(0.1,0.9), labels=c("Absent", "Present"))
  axis(2, at=c(0, 0.5, 1), labels=c("0", "0.5", "1"), cex.axis=1.5)
  mtext ("gift exchange", cex=1.25, line=0.5)
  ...
  gift_ex <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    gift_ex[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  gift_ex <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    gift_ex[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
  ...
  dowry <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    dowry[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,5])
  }
  ...
  dowry <- matrix(0, nrow=100, ncol=length(seq_f))
  for (i in 1:100) {
    dowry[i,] <- pordlogit_rev (1, phi[i,], norm_post_g[i,6])
  }
